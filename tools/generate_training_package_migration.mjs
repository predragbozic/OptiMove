...
import { sqlLegacyPlanChecksum } from "./legacy_plan_sql_signature.mjs";
...
async function findReplacementBackup(client, replacement) {
  ...
  return { ...replacement, auditChecksum: backup.normalizedChecksum || null, checksum: await sqlLegacyPlanChecksum(client, normalized), counts: normalized.counts, normalized };
}
...
async function extractPayload(client, athleteConfig) {
  ...
  const replacements = [];
  for (const replacement of plans.map((plan) => APPROVED_REPLACEMENTS.get(`${packageId}|${plan.week_start}`)).filter(Boolean)) {
    replacements.push(await findReplacementBackup(client, replacement));
  }
  return {
    packageId,
...
drop function if exists public.${fnPrefix}_legacy_plan_component_checksums(jsonb);
create function public.${fnPrefix}_legacy_plan_component_checksums(p_normalized jsonb)
returns jsonb
language sql
immutable
as $components$
...
drop function public.${fnPrefix}_legacy_plan_component_checksums(jsonb);
