...
export async function getActivityRoster(ctx, activityId) {
  return readInSnapshot(async (client) => {
    ...
    if (completionRow) {
      ...
      // the completion rested on is compared on every read. Reported, not
      // written - a read writes nothing.
      if (completionRow.status === "complete" && completionRow.input_fingerprint !== fingerprint) {
