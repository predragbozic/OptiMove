...
export const MENU_CACHE_POLICIES = {
  ...
  "training-load": {
    label: "Training load",
    policy: "cached",
    namespace: "training-load-weekly",
    rationale:
      "fix/training-load-perf-nav-results: DEVIATES from this file's own prior 'always-refresh' policy for this tab (see tests' own entry above for the original reasoning that used to apply here too). Wired into view-cache.js in this branch specifically because GET /weekly's own response is IDENTICAL across Today/Schedule/Results for the same (workspace, weekStart, filter) - the always-refresh policy meant switching those three tabs re-fetched the exact same payload three times in a row, which is the redundant-request pattern this branch was asked to fix. Context key is [...workspace parts, weekStart, filterQueryString] (trainingLoadWeeklyContextKey in training-load-data.js) - the athlete's own weekly overlay shares the SAME namespace/key shape (its own 'athlete' request-generation slot only) so it also benefits when it happens to match a coach tab already viewing the same week. TTL: reuses the default VIEW_CACHE_FRESHNESS_MS (30s) per this file's own guidance, not a bespoke number - short enough that the original staleness concern (an athlete submitting RPE, a coach checking in mid-day) only ever shows a stale view for up to 30s on a passive re-entry, and every mutation that actually changes this view explicitly invalidates BEFORE reloading (captureTrainingLoadWeeklyMutationContext/invalidateTrainingLoadWeeklyContext for a mutation confined to one week - but EVERY cached filter variant of it, since the same session can appear under several at once - RPE submit, RPE-enabled toggle; invalidateAllTrainingLoadWeeklyGenerations for one that can affect several cached weeks/filters at once - the master toggle, ownership resolution, an external schedule's status change, or a recurring/multi-date create/edit/schedule-again - all in training-load-data.js/training-load-actions.js), so mutation-driven freshness is exact regardless of the TTL. Every capture helper snapshots the account/workspace identity too, not just the week/filter, and every invalidate+reload is gated on that identity still being current - a workspace switch mid-flight is left entirely to resetTrainingLoadForWorkspaceChange, never re-targeted at whatever workspace happens to be active once the mutation resolves. loadTrainingLoad() (app.js) and the section-switch/week-nav/filter handlers (training-load-actions.js) still keep their own per-section request-generation-token guard (mirroring tests-data.js's loadTestsWeekly) alongside the cache layer's own race guard, belt-and-suspenders. The schedule-only planned-RPE master-toggle setting (loadPlannedRpeSetting) is a separate, cheap, deliberately UNcached single value - unaffected by this change, still always-refresh.",
  },
};
