...
**Implemented in 5a2** (branch `feature/activity-roster-decisions-5a2`): migration v26 (`migrations_v2/202609252000_training_load_v26_activity_roster_decisions.sql`), the five write commands (`backend/src/activityRosterCommands.js`) and the automatic `complete → needs_review`. Where 5a2 differs from the text below, section 11 says so; section 11 is authoritative for the endpoints, bodies and codes.
...
- *Recorded, but not on this session's roster* — a measured athlete whose membership periods do not cover the session (joined later, left earlier or never a member; the API does not claim which): listed in a folded group, never blocks.
...
|---|---|---|---|---|
| `GET /api/training-activity/:activityId/roster` | — | returns `revision` | `200 { activity, canonicalActivityId, completion: {status, revision, completedBy, completedAt, needsReviewCauses}, athletes: [{athleteId, name, membership: {from, to, leftAfterSession}, state, group, decision: {id, kind, reasonKey, note, decidedBy, decidedAt} \| null, flags, sourceReason, values}], recordedOutsideRoster: [...], reasons: [{key, label}], counts: {total, needsState, needsReview}, canComplete }` | `404 notFound`, `409 roster_not_applicable` |
...
- **Response fields beyond section 4:** per athlete `stateLabel`, `membership.leftAt`, `decision.label`, `decision.activityId` (the alias the decision was written on), `sourceReason.label` and `sourceReason.sourceSystem`; `counts.recordedOutsideRoster`; `viewer.basis`. When several current decisions agree (the same decision on two aliases before a merge), `decision` is the latest of them.
- **Completion log** (external review of PR #122): a log row's activity must be canonical and team-owned (`assert_canonical_team_activity`, like every other new table), and at commit its request must belong to the same activity or to its canonical alias set.
...
- **Known consequence of `valid_from = starts_at`.** An athlete counts from the moment he was added in OptiMove. A GPEXE session from before a team's athletes were added lists its measured athletes under *Recorded, but not on this session's roster* and leaves the roster short; this matters for importing older sessions (see CURRENT_STATE).

---

## 11. As built in 5a2 (differences in detail from the text above)

Migration v26, `backend/src/activityRosterCommands.js`, routes in `backend/src/routes/trainingActivity.js`, runbook `docs/runbooks/activity-roster-v26.md`, rollback `docs/runbooks/activity-roster-v26-rollback.sql`, tests `backend/tests/activity-roster-5a2.test.mjs`.

### Endpoints (owner's 5a2 order; they replace the `…/completion/…` paths of section 4)

| Endpoint | Body | Success |
|---|---|---|
| `PUT /api/training-activity/:activityId/roster/:athleteId/decision` | `{ kind, reasonKey?, note?, expectedDecisionId, requestKey }` | `200 { decision, athlete, completion }` |
| `DELETE /api/training-activity/:activityId/roster/:athleteId/decision` | `{ expectedDecisionId, requestKey }` (a uuid: the decision being removed) | `200 { decision (kind cleared), athlete, completion }` |
| `POST /api/training-activity/:activityId/roster/decisions` | `{ kind, reasonKey?, note?, athletes: [{ athleteId, expectedDecisionId }], requestKey }` | `200 { decisions, athletes, completion }` |
| `POST /api/training-activity/:activityId/roster/complete` | `{ expectedRevision, expectedFingerprint, requestKey }` | `200 { completion }` |
| `POST /api/training-activity/:activityId/roster/reopen` | `{ expectedRevision, reason, requestKey }` | `200 { completion }` |

- `kind`: `did_not_participate` (reason required) or `participated_no_values` (no reason). `manual_values` / `estimated` answer `409 kind_not_available` until 5b. `note`: trimmed, at most 500, empty = none.
- `expectedDecisionId` matches one of the athlete's current decisions across the alias set; `null` matches "none, or the current one is `cleared`" (the read shows no decision then). On a merge conflict either current id matches, and the new decision supersedes every current one.
- **`expectedFingerprint` is new.** The read returns `rosterFingerprint` (the roster the coach looked at). A change that bumps no revision (a measurement or a new member on a session that is not complete) makes Complete answer `409 roster_changed`. The owner's order says a stale revision and a changed fingerprint both refuse Complete; the body of section 4 could not carry the second.
- **Reopen takes `reason`** (required, 1–500, trimmed; stored in the log's `detail`), per the owner's 5a2 order ("an explicit command with a reason"), not an optional `note`.
- Stable errors: `400 invalid_request_key / invalid_kind / reason_required / reason_not_allowed / unknown_reason / note_too_long / invalid_note / invalid_expected_decision_id / invalid_athletes / too_many_athletes / duplicate_athlete / invalid_expected_revision / invalid_expected_fingerprint / reason_too_long / invalid_reason`; `404 notFound` (identical body for everything outside the active-workspace path, a malformed id included); `403 not_a_team_coach` (the right disappeared between the request's start and `lock_activity_decider`); `409 request_key_reused / activity_superseded {canonicalActivityId} / roster_not_applicable / not_on_roster / decision_changed {current} / measured_record_exists {current} / kind_not_available / nothing_to_clear {current} / bulk_conflict {failed: [{athleteId, error, current}]} / revision_changed {revision} / already_complete / roster_changed {revision} / roster_empty / roster_incomplete {needsStateAthleteIds, revision} / not_complete`; `503 roster_busy` (a lock wait over 15 s, nothing written) / `outcome_unknown` (message "Not sure the change was saved. Try again with the same requestKey; it will not be saved twice."); `500 internal_error` (message "Nothing was saved. Try again.": a certain rollback — anything before the COMMIT, or a COMMIT the server itself refused). A COMMIT that was applied but whose answer was lost is answered `200` from the stored result. Database text never reaches the client.
- New beyond section 4: `nothing_to_clear`, `bulk_conflict`, `already_complete`, `roster_changed`, `roster_empty`, `roster_busy`, `outcome_unknown`.

### Transaction, locks and idempotency

- One transaction per request, `lock_timeout = 15s`. Order: resolve the canonical activity, the team and the caller's basis exactly as the read does (404 before any lock) → `lock_activity_decider` → `lockTeamForImport` → *Complete only:* `lock_roster_team(team, exclusive)` → the alias set's activity rows `FOR KEY SHARE`, ascending (re-read until stable) → *Complete only:* `lock_roster_completions(canonical, exclusive)` → completion rows `FOR UPDATE`, ascending → the request key across the alias set → checks → writes → the request row → COMMIT.
- **Why two new advisory locks.** The order of 3.6 serialises decisions with imports; it did not stop a writer without the team lock (a late link, a reparent, a membership change) from committing next to a Complete that could not yet see it. Every roster input trigger now takes the activity's completion lock shared (a membership change also the team's roster lock shared) before it looks at the completion row, and Complete takes both exclusive: an uncommitted change is either waited for and seen (Complete then answers `roster_changed`), or waits for Complete and then downgrades it. This also covers a session with no completion row yet.
- **Idempotency.** The request hash is sha256 of the canonical body (sorted keys, the athlete list sorted, the note trimmed), without the activity id: the key is scoped to the alias set, so the same key and body on any alias of the session returns the stored answer (after re-authorization, before any `activity_superseded`). A refused request writes nothing and keeps its key free. A key stored by another user is `request_key_reused`, never that user's answer.
- Write order kept from 5a1: current decisions are superseded first with the new ids known in advance, then the new decisions in one INSERT (one revision per request, a bulk included), the request row last.
- A command that finds a `complete` session whose fingerprint changed first writes the downgrade (`input_changed`); Complete and Reopen compare `expectedRevision` with the stored revision before that.
- **The COMMIT outcome** (external review of PR #123, the GPEXE approval's pattern): the COMMIT's answer is awaited at most 15 s. A certain rollback is only a refusal answered by the server itself (a pg `DatabaseError`) outside `08xxx`, `57Pxx` and `40003 statement_completion_unknown`. Everything else — those SQLSTATEs, a socket or driver error, the 15 s timeout — is an unknown outcome: the connection is destroyed first (never returned to the pool, which also ends a COMMIT still waiting on it), then the request row is looked for on a new connection, the whole check (getting the connection and the query) bounded to 5 s; a late connection is destroyed on arrival. Found → `200` with the stored result; not found, failed or timed out → `503 outcome_unknown`.

### The automatic `needs_review` (layer 1 of 3.4)

- Statement-level triggers with transition tables, one per input (table in the v26 runbook); the fan-out locks completion rows in ascending activity id per statement. A guard (`optimove.roster_change_active`) stops re-entry.
- `complete → needs_review` clears `completed_by_*`, `completed_at` and `input_fingerprint` on the row (the v25 check requires it); who completed and when stays in the log (`detail.completedByUserId`, `detail.completedAt`).
- On `needs_review` a new cause is added (revision + 1, one log row); a cause already present writes nothing — no duplicate log rows for more of the same change. Nothing returns to `complete` without the command.
- `not_complete` (or no row) is left alone by system changes; a coach decision always bumps the revision (and creates the row at revision 1). The completion log therefore has one row per revision: the roster's revision history.
- **Never blocking the writer:** the downgrade has no business rule that could refuse a valid write. If it fails technically (e.g. its log row cannot be written), the writer's statement fails and its whole transaction rolls back — a change never commits next to a `complete` it did not downgrade (test 51 injects exactly that failure).
- Scope choices: any occasion insert for a linked participant downgrades (also a manual one); a participant merge counts as `activity_merged`; a reparent as `roster_changed` on both sessions; a supersede downgrades the survivor only, with **`activity_merged`** (section 3.0 said `roster_changed`; the cause list has `activity_merged` for exactly this), and the superseded activity's own row stays as history; a change of the session's `started_at` / `occurred_local_date` / `timezone_snapshot` (e.g. `correct_confirmed_activity_fields()`) is `roster_changed`; a membership change touches only sessions that are complete or under review.
- After the alias set is locked, a canonical activity that differs from the one resolved before the locks (a merge committed in between) is authorized again; a session now owned by another team is the identical 404.
- **The club is part of the right** (external review of PR #123): v26 redefines `training.lock_activity_decider` so that every basis holds, in order, the team (active) → its club (active) → the role row → the user row, all `FOR SHARE`. Archiving the parent club therefore waits for a decision in flight of a team coach, a club admin or a platform admin, and an archived club refuses every basis (42501). No cycle: a club archive is one `UPDATE` of the club row, a team archive one `UPDATE` of the team row (no trigger on either), and neither then waits for another row the decider holds. The read and the commands treat a team of an archived club like an archived team (the identical 404); a request that resolved before the archive committed and then waited for it gets `403 not_a_team_coach` with nothing written.

### Database rules raw SQL cannot bypass (v26)

One log row per revision (unique index; checked at commit); a log row describes the completion row it follows and continues the previous revision's status; each cause allows only its transitions; `completed` / `reopened` carry the request, the user and a basis the user holds (`lock_activity_decider`), `reopened` a reason; a completion row starts at revision 1 and becomes complete only over a non-empty roster in which nobody needs a state (`training.activity_roster_needs_state`, the SQL twin of the read's derivation, tested to agree); a new decision is the only current one of its athlete in the alias set. A raw decision insert gets its revision bump and log row from the trigger. The fingerprint itself cannot be verified in SQL; a wrong one is caught by the read (`input_changed`).

### Left for later

- **5a3:** the roster UI (table and cards, Set state, bulk, Change, flags, Complete, the status on Activities and Imports, the error texts of section 5.9).
- **5b:** manual values and estimates (`manual_values`, `estimated`, `metric_estimations`, the coach-owned event, the Dashboards source policy, the Activities labels).
- **5c:** `change_pending` from the adapter and the per-session confirmation of a later measurement.
- Still open from 5a1: the roster of a session from before its athletes were added in OptiMove, and memberships archived and restored before v25 (CURRENT_STATE, Open risks).
